...
const CategoryPage = () => {
  ...
  const openEdit = (cat: Category) => {
    setEditItem(cat);
    setForm({ name: cat.name, color: cat.color || "#000000", status: cat.status, image: cat.image });
    setImgFile(null);
  ...
  const handleDelete = async (id: string) => {
    if (!confirm("Delete this category? All its sub-categories will also be deleted.")) return;
    try {
