...
function MyAccount() {
  ...
  const handlePwdChange = (e: React.ChangeEvent<HTMLInputElement>) =>
    setPwdFields({ ...pwdFields, [e.target.name]: e.target.value });
  ...
  const handleSave = async () => {
    setIsLoading(true);
    try {
      const res = await putData("/api/user/update-user", {
      ...
      context.alertBox({ type: "error", msg: "Failed to update profile" });
    }
    setIsLoading(false);
  };
  ...
  const handleChangePassword = async () => {
    if (
      !pwdFields.oldPassword ||
      !pwdFields.newPassword ||
      !pwdFields.confirmPassword
    ) {
      context.alertBox({
        type: "error",
    ...
    if (pwdFields.newPassword !== pwdFields.confirmPassword) {
      context.alertBox({ type: "error", msg: "New passwords do not match!" });
      return;
    }
    ...
    try {
      const res = await putData("/api/user/reset-password", {
        ...
        confirmPassword: pwdFields.confirmPassword,
      });
      if (res?.success) {
        context.alertBox({
          type: "success",
      ...
      context.alertBox({ type: "error", msg: "Failed to change password" });
    }
    setPwdLoading(false);
  };
  ...
  const handleAddrSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = editingAddr
        ? await putData(
            `/api/address/${editingAddr._id}`,
            addrForm as unknown as Record<string, unknown>,
          )
        : await postData(
            "/api/address/add",
            addrForm as unknown as Record<string, unknown>,
      ...
      if (res?.success) {
        context.alertBox({
          type: "success",
          msg: editingAddr ? "Address updated!" : "Address added!",
        });
        setShowModal(false);
        fetchAddresses();
      } else
        context.alertBox({
          type: "error",
          msg: res?.message || "Failed to save address",
        });
    } catch {
      context.alertBox({ type: "error", msg: "Failed to save address" });
    }
  ...
  const handleDeleteAddr = async (id: string) => {
    ...
    try {
      ...
      if (res?.success) {
        context.alertBox({ type: "success", msg: "Address deleted!" });
        fetchAddresses();
      }
    } catch {
      context.alertBox({ type: "error", msg: "Failed to delete address" });
    }
  };
            ...
            <div className="bg-white rounded-md shadow-sm p-5">
              ...
              <div className="flex gap-3 mt-5">
                <button
                  onClick={handleSave}
                  disabled={isLoading}
                  className="bg-[#e8623d] text-white px-8 py-3 rounded-md text-[14px] font-medium hover:bg-[#d95734] transition disabled:opacity-60"
                >
              ...
              <div className="mt-8 pt-6 border-t border-gray-200">
                <div className="flex items-center justify-between mb-4">
                  ...
                  <button
                    ...
                    className="flex items-center gap-1 bg-[#e8623d] text-white px-4 py-2 rounded-md text-[13px] font-medium hover:bg-[#d95734] transition"
                  >
                ...
                {addresses.length === 0 ? (
                  ...
                  <div className="grid grid-cols-1 gap-3">
                    {addresses.map((addr) => (
                      <div
                        key={addr._id}
                        className={`border rounded-md p-4 relative ${addr.isDefault ? "border-[#e8623d] bg-[#fdf9f8]" : "border-gray-200"}`}
                      >
                        {addr.isDefault && (
                          <span className="absolute top-3 right-3 text-[11px] bg-[#e8623d] text-white px-2 py-0.5 rounded-full">
                            Default
                          </span>
                        )}
                        ...
                        <div className="flex items-center gap-3 mt-3">
                          {!addr.isDefault && (
                            <button
                              ...
                              className="text-[12px] text-[#e8623d] border border-[#e8623d] px-3 py-1 rounded-md hover:bg-[#fdf1ee] transition"
                            >
                              Set as Default
                            </button>
                          )}
                          <button
                            ...
                            className="flex items-center gap-1 text-[12px] text-gray-600 border border-gray-300 px-3 py-1 rounded-md hover:bg-gray-50 transition"
                          >
                            <MdEdit className="text-[14px]" /> Edit
                          </button>
                          <button
                            onClick={() => handleDeleteAddr(addr._id)}
                            className="flex items-center gap-1 text-[12px] text-red-500 border border-red-200 px-3 py-1 rounded-md hover:bg-red-50 transition"
                          >
      ...
      {showModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
          <div className="bg-white rounded-lg shadow-xl w-full max-w-lg mx-4 p-6">
            <h3 className="text-[17px] font-semibold text-gray-800 mb-5">
            ...
            >
              <input
                ...
                required
                className="col-span-2 border border-gray-300 rounded-md px-3 py-2.5 text-[13px] outline-none focus:border-[#e8623d]"
              />
              <input
                name="phone"
                placeholder="Phone *"
              ...
              <input
                ...
                onChange={handleAddrChange}
                required
                className="border border-gray-300 rounded-md px-3 py-2.5 text-[13px] outline-none focus:border-[#e8623d]"
              />
              <input
                name="state"
                placeholder="State *"
              ...
              <div className="col-span-2 flex gap-3 mt-2">
                <button
                  ...
                  className="bg-[#e8623d] text-white px-7 py-2.5 rounded-md text-[13px] font-medium hover:bg-[#d95734] transition disabled:opacity-60"
                >
                  {addrLoading
                    ? "Saving..."
                    : editingAddr
                ...
                <button
                  ...
                  className="border border-gray-300 text-gray-600 px-7 py-2.5 rounded-md text-[13px] font-medium hover:bg-gray-50 transition"
                >
                  Cancel
                </button>
              </div>
